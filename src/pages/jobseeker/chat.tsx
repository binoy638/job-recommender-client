...
const fetchChat = async () => {
  const { data } = await JobSeekerAPI.getChat();
  return data.chat;
};
...
const ChatBody = ({ activeChat }: { activeChat: Chat | null }) => {
  ...
  const { mutate } = useMutation(JobSeekerAPI.sendMessage, {
    onSuccess: () => {
      ...
      <div className="w-full">
        <div className="relative flex items-center border-b border-gray-300 p-3">
          <Avatar color="cyan" radius="xl">
            {getInitial(
              activeChat.employer.firstName,
              activeChat.employer.lastName
            )}
          </Avatar>
          <span className="ml-2 block font-bold text-gray-600">
            {activeChat.employer.firstName} {activeChat.employer.lastName}
          </span>
        </div>
...
const JobSeekerChat = ({ chat }: { chat: Chat[] }) => {
              ...
              {data.map((chat_) => {
                return (
                  ...
                  >
                    <Avatar color="cyan" radius="xl">
                      {getInitial(
                        chat_.employer.firstName,
                        chat_.employer.lastName
                      )}
                    </Avatar>
                    <div className="w-full pb-2">
                      <div className="flex justify-between">
                        <span className="ml-2 block font-semibold text-gray-600">
                          {chat_.employer.firstName} {chat_.employer.lastName}
                        </span>
